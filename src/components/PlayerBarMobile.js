...
export const PlayerBarMobile = () => {
  ...
  const {wins, playerTurn, winner, paused} = useContext(GameContext)
    ...
    <div className=" relative flex lg:hidden justify-between">
      <div className="flex h-[81px] sm:h-[100px] bg-white border-4 rounded-[20px] shadoww justify-between w-[47%] sm:w-[42%] py-2 pr-8 sm:ml-8">
        <div className="flex items-center ml-[-20px]">
          <img src={player1} className={`w-[54px] h-[59px] ${playerTurn === 1 && winner === 0 && paused === false ? "bouncey" : ""}`} alt=""></img>
        </div>
      ...
      <div className="sm:flex items-center">
        ...
        <div className="flex items-center mr-[-20px]">
          <img src={player2} className={`w-[54px] h-[59px] ${playerTurn === 2 && winner === 0 && paused === false ? "bouncey" : ""}`} alt=""></img>
        </div>
      </div>
    </div>
  )
